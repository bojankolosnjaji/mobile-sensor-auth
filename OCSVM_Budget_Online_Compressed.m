function [ X_all,y,alpha_new,K_new, output ] = OCSVM_Budget_Online_Compressed( X, X_new, K, alpha, params, strategy)
...
if (sum(alpha_new~=0)>budget)
  %  ['rationalizing']
    if (strategy==1) % deletion
        alpha_new(ind_min_alpha)=[];
        K_new(ind_min_alpha,:) = [];
        K_new(:,ind_min_alpha) = [];
        X_all(ind_min_alpha,:) = [];

    elseif (strategy==2) % projection
        K_p = [K_new(1:ind_min_alpha-1,:); K_new(ind_min_alpha+1:size(K_new,1),:)];
